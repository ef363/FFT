...
class TestILT(unittest.TestCase):
	...
	fhat_mat = ilt.make_ILT_input(fhat, delta, M)
        fx = np.zeros(M)
        np.testing.assert_allclose(ilt.ILT(fhat_mat, delta, M), fx, atol=1e-2)
    ...
    #     np.testing.assert_allclose(fft.ILT(fhat_mat, delta, M), fx, atol=1e-2)
...
allTests = unittest.TestSuite([suiteILT, suiteMakeInputs])
